...
class HrLeaveRestriction(models.Model):
    ...
    def action_approve(self):
        if self.env.context.get('is_an_admin'):
            self.env.context.clear()
            mail_template = self.env.ref('leave_restriction_if_team_is_understaffed.leave_approved_mail')
            ...
            if len(self.employees_on_leave) / total_employees > 0.5:
                return {
                    ...
                    'context': {
                        ...
                        'leave_id': self.id,
                        'is_an_admin': self.env.user.has_group('hr_holidays.group_hr_holidays_manager')
                    }
                }
        return super().action_approve()
    def action_refuse(self):
        template = self.env.ref('leave_restriction_if_team_is_understaffed.leave_refused_mail')
